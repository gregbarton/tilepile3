...
public class MuralFrame extends JInternalFrame implements MainStationRemoteListener {
    ...
    public MuralFrame(
        ...
        MainStationRemoteImpl mainStationRemote,
        final DataManagerRemote dataManager, 
        GraphicsConfiguration graphicsConfiguration) throws TilepileException, RemoteException {
        setMural(mural);
                ...
                if(gridActive) {
                    ...
                    for(int i = 0; i < gridX; i++) {
                        ...
                        for(int j = 0; j < gridY; j++) {
                            ...
                            Shape sha = new TextLayout(as.getIterator(), frc).getOutline(
                                AffineTransform.getTranslateInstance(
                                    (rec.x + (rec.width / 2)) - ((num.length() * font.getSize2D()) / 3), 
                                    rec.y + (rec.height / 2) + (font.getSize2D() / 3)
                                )
                            );
                            ...
                            try {
                                if (states.isFinished(gridX, gridY)) {
                                    ...
                                    int decision = JOptionPane.showConfirmDialog(
                                        panel,
                                        "This section has already been finished.  Continue?");
                                ...
                                dialog.setBounds(
                                    pfBounds.x + (pfBounds.width / 4),
                                    pfBounds.y + (pfBounds.height / 4),
                                    pfBounds.width / 2,
                                    pfBounds.height / 2);
                                            ...
                                            stationButton.addActionListener(new ActionListener() {
                                                public void actionPerformed(ActionEvent e) {
                                                    ...
                                                    try {
                                                        ...
                                                        stationRemote.setMuralLocation(
                                                            getMainStation().getName(),
                                                            muralLocation
                                                        );
                                ...
                                if (stationButtons.size() > 0) {
                                    ...
                                    JOptionPane.showMessageDialog(
                                        parentFrame, 
                                        "No available substations on network",
                                        "Open a substation",
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                chooser.addChoosableFileFilter(new FileFilter() {
                    public boolean accept(File f) {
                        ...
                        return f.isDirectory()
                            || (f.isFile() && f.getName().equals(
                                mural.getName() + DataConstants.DATA_FILE_EXTENSION));
                    }
                    ...
                    try {
                        ...
                        if(oldStates.add(newStates)) {
                            ...
                            JOptionPane.showInternalMessageDialog(
                                MuralFrame.this,
                                "File " + chooser.getSelectedFile().getName() + " incompatible with states from " + mural.getName(),
                ...
                for(int i = 0; i < panels.length; i++) {
                    for(int j = 0; j < panels[i].length; j++) {
                        if(blockedItem.getState()) {
                            panels[i][j].activateShowStates(MuralSectionPanel.SHOW_LOCKED);
                        } else {
                            panels[i][j].deactivateShowStates(MuralSectionPanel.SHOW_LOCKED);
                        }
                    }
                }
                muralPanel.repaint();
                ...
                for(int i = 0; i < panels.length; i++) {
                    for(int j = 0; j < panels[i].length; j++) {
                        if(fabricatedItem.getState()) {
                            panels[i][j].activateShowStates(MuralSectionPanel.SHOW_FINISHED);
                        } else {
                            panels[i][j].deactivateShowStates(MuralSectionPanel.SHOW_FINISHED);
                        }
                    }
                }
                muralPanel.repaint();
                ...
                for(int i = 0; i < panels.length; i++) {
                    for(int j = 0; j < panels[i].length; j++) {
                        if(stockItem.getState()) {
                            panels[i][j].activateShowStates(MuralSectionPanel.SHOW_STOCK);
                        } else {
                            panels[i][j].deactivateShowStates(MuralSectionPanel.SHOW_STOCK);
                        }
                    }
                }
                muralPanel.repaint();
