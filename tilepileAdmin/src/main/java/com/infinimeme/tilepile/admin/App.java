...
public class App {
	...
	public static void main(String[] args) {
		...
		try {
			DATA_MANAGER = DataManagerDiscovery
					.getDataManager(DataManagerDiscovery.DiscoverableType.ADMIN);

			JFrame frame = new JFrame("Tilepile Admin");
			GraphicsEnvironment ge = GraphicsEnvironment
					.getLocalGraphicsEnvironment();
			GraphicsDevice[] gds = ge.getScreenDevices();
			Rectangle screenBounds = gds[0].getDefaultConfiguration()
					.getBounds();
	...
	private static final void addMuralMenuItems(final String name) {
		...
		openMuralMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				try {
					desktop.add(new MuralEditor(DATA_MANAGER.getMural(name),
							DATA_MANAGER));
				} catch (RemoteException re) {
					TilepileUtils.exceptionReport(re);
				}
		...
		deleteMuralMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				int option = JOptionPane.showConfirmDialog(desktop,
						"Are you sure?", "Really delete " + name + "?",
			...
			public void actionPerformed(ActionEvent event) {
					...
					public boolean accept(File f) {
						...
						return f.isDirectory()
								|| (f.isFile() && f.getName().endsWith(
										DataConstants.DATA_FILE_EXTENSION));
					}
				...
				chooser.addChoosableFileFilter(new FileFilter() {
					...
					public String getDescription() {
						...
						return name + DataConstants.DATA_FILE_EXTENSION
								+ " States data file";
					}
				});
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						States newStates = DataUtils.readFile(
								chooser.getSelectedFile(), States.class);
							...
							JOptionPane.showInternalMessageDialog(desktop,
									"File "
											+ chooser.getSelectedFile()
													.getName()
											+ " incompatible with states from "
											+ name,
									"States files incompatible!",
			...
			public void actionPerformed(ActionEvent event) {
				try {
					desktop.add(new PaletteEditor(
							DATA_MANAGER.getPalette(name), DATA_MANAGER));
				} catch (RemoteException re) {
					TilepileUtils.exceptionReport(re);
				}
			}
	 ...
	 *             DOCUMENT ME!
	 */
	private static final JMenuBar createMenu(final JDesktopPane desktop,
			final JFrame frame) throws TilepileException {
		...
		resetItem.setAccelerator(KeyStroke.getKeyStroke(new Character('r'),
		...
		importMuralMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							ImportMuralDialog dialog = new ImportMuralDialog(
									frame);
							dialog.setLocationRelativeTo(frame);
							dialog.setVisible(true);
						} catch (RemoteException re) {
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					...
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						...
						Mural mural = MuralFactory.make(
								chooser.getSelectedFile(), DATA_MANAGER);
		...
		addPaletteMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				try {
					String name = JOptionPane.showInputDialog(desktop,
							"What is the new palette name?");
					...
					if (DATA_MANAGER.containsPalette(name)) {
						JOptionPane.showMessageDialog(desktop, "Palette name "
								+ name + " already used",
								"Palette name in use",
								JOptionPane.ERROR_MESSAGE);
					}

					String num = JOptionPane.showInputDialog(desktop,
					...
					Palette palette = PaletteFactory.make(name,
				...
				try {
					...
					if ((name != null) && (!DATA_MANAGER.containsPalette(name))) {
						...
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							...
							Palette palette = PaletteFactory.makeFromPhotoshopPalette(name,
									chooser.getSelectedFile());
						...
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							...
							if (image == null) {
								JOptionPane.showMessageDialog(desktop,
										"Cannot read image format: " + imageFile,
										"Invalid image file",
										JOptionPane.ERROR_MESSAGE);
								return;
							}
							...
							if(palette == null) {
								JOptionPane.showMessageDialog(desktop,
										"Cannot convert image to palette: " + imageFile,
										"Invalid image file",
										JOptionPane.ERROR_MESSAGE);
								return;
							} else {
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					...
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						...
						Palette palette = PaletteFactory.make(chooser
								.getSelectedFile());
	...
	private static class MergePalettesDialog extends JDialog {
		...
		public MergePalettesDialog(final JFrame frame) throws RemoteException {
			...
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(paletteNameLabel)
					.addComponent(mergeNamesLabel)
					.addComponent(mergeColorsLabel)
					.addComponent(palettesLabel)
					.addComponent(cancelButton));
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(paletteNameText)
					.addComponent(mergeNamesCheck)
					.addComponent(mergeColorsCheck)
					.addComponent(palettesList)
					.addComponent(okButton));
			layout.setHorizontalGroup(hGroup);

			GroupLayout.SequentialGroup vGroup = layout.createSequentialGroup();
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(paletteNameLabel).addComponent(paletteNameText));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(mergeNamesLabel).addComponent(mergeNamesCheck));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(mergeColorsLabel).addComponent(mergeColorsCheck));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
				.addComponent(palettesLabel).addComponent(palettesList));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
	...
	private static class LoadMuralDialog extends JDialog {
		...
		public LoadMuralDialog(final JFrame frame) throws RemoteException {
			...
			muralName.setInputVerifier(new InputVerifier() {
				...
				public boolean verify(JComponent input) {
					...
					if (used) {
						...
						JOptionPane.showMessageDialog(LoadMuralDialog.this,
								"Mural name " + name + " already in use",
								"Mural Name Error", JOptionPane.ERROR_MESSAGE);
					} else {
			...
			JLabel paletteNameLabel = new JLabel("Palette Name");
			final JComboBox paletteName = new JComboBox(DATA_MANAGER
					.namesOfPalette().toArray());
			...
			gridWidth.setInputVerifier(new InputVerifier() {
				...
				public boolean verify(JComponent input) {
					...
					try {
						...
						field.setBackground(Color.RED);
						JOptionPane.showMessageDialog(LoadMuralDialog.this,
								"Please enter an integer", "Grid Width Error",
								JOptionPane.ERROR_MESSAGE);
					}
					return parsed;
				}
			...
			gridHeight.setInputVerifier(new InputVerifier() {
				...
				public boolean verify(JComponent input) {
					...
					try {
						...
						field.setBackground(Color.RED);
						JOptionPane.showMessageDialog(LoadMuralDialog.this,
								"Please enter an integer", "Grid Height Error",
								JOptionPane.ERROR_MESSAGE);
					}
					return parsed;
				}
			...
			okButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (muralFile == null) {
						JOptionPane.showMessageDialog(desktop,
								"Please select a mural backup file",
								"No mural backup file selected",
								JOptionPane.ERROR_MESSAGE);
					} else {

						try {
							Mural mural = MuralFactory.make(
									muralName.getText(), Integer
											.parseInt(gridWidth.getText()),
									Integer.parseInt(gridHeight.getText()),
									muralFile, DATA_MANAGER
			...
			GroupLayout.SequentialGroup hGroup = layout.createSequentialGroup();
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(muralNameLabel)
					.addComponent(paletteNameLabel)
					.addComponent(imageFileLabel).addComponent(gridWidthLabel)
					.addComponent(gridHeightLabel).addComponent(cancelButton));
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(muralName).addComponent(paletteName)
					.addComponent(imageFileButton).addComponent(gridWidth)
					.addComponent(gridHeight).addComponent(okButton));
			layout.setHorizontalGroup(hGroup);

			GroupLayout.SequentialGroup vGroup = layout.createSequentialGroup();
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(muralNameLabel).addComponent(muralName));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(paletteNameLabel).addComponent(paletteName));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(imageFileLabel).addComponent(imageFileButton));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(gridWidthLabel).addComponent(gridWidth));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(gridHeightLabel).addComponent(gridHeight));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(cancelButton).addComponent(okButton));
			layout.setVerticalGroup(vGroup);
	...
	private static class ImportMuralDialog extends JDialog {
		...
		private static final boolean checkForm(final JCheckBox box,
			...
			if (nameUsed && !replaced) {
				...
				int results = JOptionPane.showConfirmDialog(box, "Mural name '"
						+ name + "' already in use.  Replace?",
						"Mural Name Error", JOptionPane.YES_NO_OPTION);
				...
				int results = JOptionPane.showConfirmDialog(box, "Mural name '"
						+ name
						+ "' is NOT used, but replace is selected.  Deselect?",
						"Mural Name Error", JOptionPane.YES_NO_OPTION);
				if (results == JOptionPane.YES_OPTION) {
		...
		public ImportMuralDialog(final JFrame frame) throws RemoteException {
			...
			gridWidth.setInputVerifier(new InputVerifier() {
				...
				public boolean verify(JComponent input) {
					...
					try {
						...
						parsed = false;
						field.setBackground(Color.RED);
						JOptionPane.showMessageDialog(ImportMuralDialog.this,
								"Please enter an integer", "Grid Width Error",
								JOptionPane.ERROR_MESSAGE);
					}
					return parsed;
			...
			final UnitField muralHeight = new UnitField(0.0,
					UnitField.Units.FEET);
			...
			replace.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					...
					boolean rep = ((JCheckBox) replace).isSelected();

					paletteName.setEnabled(!rep);
					gridWidth.setEnabled(!rep);
					gridHeight.setEnabled(!rep);

					boolean ptt = ((JCheckBox) pixelToTile).isSelected();
			...
			okButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (imageFile == null) {
						JOptionPane.showMessageDialog(desktop,
								"Please select a mural image file",
								"No mural image file selected",
								JOptionPane.ERROR_MESSAGE);
					} else {
						...
						try {
							...
							if (replace.isSelected()) {
								...
								if (DATA_MANAGER.namesOfMural().contains(name)) {
									...
									Palette palette = DATA_MANAGER.getPalette(m
											.getPaletteName());
									mural = MuralFactory.replace(m, image,
											palette);
								} else {
									JOptionPane
											.showMessageDialog(
													...
													"'"
															+ name
															+ "' is not an existing mural.  Please enter an existing mural name or deselect 'Replace'",
													"Invalid mural name",
													JOptionPane.ERROR_MESSAGE);
								}
							} else {
								...
								if(selectedPaletteName == null || "".equals(selectedPaletteName.trim())) {
									if (JOptionPane.showConfirmDialog(
											desktop,
											"Palette name is blank.  Do you wish to create a palette from the image?",
											"Blank Palette",
											JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
										
										selectedPaletteName = JOptionPane.showInputDialog(desktop,
												"What is the new palette name?", name + "-autoPalette");

										if (name == null) {
											JOptionPane
											.showMessageDialog(
													desktop,
													"Cancelling mural import due to blank palette",
													"Invalid Palette",
													JOptionPane.ERROR_MESSAGE);
											return;
										}
								...
								Palette palette = DATA_MANAGER
										.getPalette(selectedPaletteName);

								if (pixelToTile.isSelected()) {

									mural = MuralFactory.make(name, Integer
											.parseInt(gridWidth.getText()),
											Integer.parseInt(gridHeight
													.getText()), image, palette, useBackupColors.isSelected());

								} else {

									double height = muralHeight
											.getAmountInInches();
									double tile = tileSize.getAmountInInches();
									double grout = groutSize
											.getAmountInInches();

									if (JOptionPane.showConfirmDialog(
											desktop,
											String.format(
													"Your mural will be %f feet tall and %f feet wide. Is this correct?",
													(height / 12),
													(((height / image
															.getHeight()) * image
															.getWidth()) / 12)),
			...
			GroupLayout.SequentialGroup hGroup = layout.createSequentialGroup();
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(muralNameLabel).addComponent(replaceLabel)
					.addComponent(paletteNameLabel)
					.addComponent(imageFileLabel).addComponent(gridWidthLabel)
					.addComponent(gridHeightLabel)
					.addComponent(pixelToTileLabel)
					.addComponent(useBackupColorsLabel)
					.addComponent(muralHeightLabel).addComponent(tileSizeLabel)
					.addComponent(groutSizeLabel).addComponent(okButton));
			hGroup.addGroup(layout.createParallelGroup()
					.addComponent(muralName).addComponent(replace)
					.addComponent(paletteName).addComponent(imageFileButton)
					.addComponent(gridWidth).addComponent(gridHeight)
					.addComponent(pixelToTile).addComponent(useBackupColors).addComponent(muralHeight)
					.addComponent(tileSize).addComponent(groutSize)
					.addComponent(cancelButton));
			layout.setHorizontalGroup(hGroup);
			...
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(muralNameLabel).addComponent(muralName));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(replaceLabel).addComponent(replace));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(paletteNameLabel).addComponent(paletteName));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(imageFileLabel).addComponent(imageFileButton));
			...
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(gridHeightLabel).addComponent(gridHeight));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(pixelToTileLabel).addComponent(pixelToTile));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
				.addComponent(useBackupColorsLabel).addComponent(useBackupColors));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(muralHeightLabel).addComponent(muralHeight));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(tileSizeLabel).addComponent(tileSize));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
					.addComponent(groutSizeLabel).addComponent(groutSize));
			vGroup.addGroup(layout.createParallelGroup(Alignment.BASELINE)
			...
			amountField.setInputVerifier(new InputVerifier() {
				...
				public boolean verify(JComponent input) {
					...
					try {
						...
						parsed = false;
						field.setBackground(Color.RED);
						JOptionPane.showMessageDialog(UnitField.this,
								"Please enter a number", "Unit Field Error",
								JOptionPane.ERROR_MESSAGE);
					}
					return parsed;
				}
