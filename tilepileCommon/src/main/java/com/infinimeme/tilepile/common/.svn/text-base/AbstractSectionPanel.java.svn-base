...
public abstract class AbstractSectionPanel extends JPanel {
     ...
     */
    private static final void drawNumber(
        String name,
        Color color,
        Graphics2D g,
        Font font,
        ...
        String num = name.substring(0, 3);

        if (numTextLayoutMap.containsKey(num)) {
            tl = (TextLayout) numTextLayoutMap.get(num);
        } else {
        ...
        Shape sha = tl.getOutline(AffineTransform.getTranslateInstance((x + (width / 2))
            - ((num.length() * font.getSize2D()) / 3), y + (height / 2) + (font.getSize2D() / 3)));
    ...
    private static final Font makeNumFont(double width, double height) {
        ...
        // Make a font that is guarenteed to fit within the height and width
        // and look "good"
        return new Font("monospaced", Font.PLAIN, (int) (Math.min(width, height) / 2.5));
    }
}
