...
public class DataManagerDiscovery {
    ...
    private static final InetAddress getDiscoveryAddress()
    ...
    public static final InetAddress discover(DiscoverableType type) throws UnknownHostException {
        ...
        try {
            ...
            byte[] addressBytes = TilepileUtils
    ...
    public static final DataManagerRemote getDataManager(DiscoverableType type)
        ...
        String remoteName = "//"
            + discover(type).getHostAddress()
            + "/"
            + DataManagerRemote.REMOTE_NAME;
                ...
                try {
                    ...
                    while (!serverDiscovered) {
                        ...
                        System.arraycopy(
                            receivedBytes,
                            1,
                            receivedAddressBytes,
                            0,
                            receivedAddressBytes.length);

                        InetAddress address = InetAddress
                            .getByAddress(receivedAddressBytes);

                        byte type = receivedBytes[0];

                        if (type == DiscoverableType.SERVER.getByte()) {
                            if (address.equals(TilepileUtils
                                .getDefaultLocalAddress())) {
                                TilepileUtils
                                    .logInfo("Server message loopback");
                            } else {
                                serverDiscovered = true;
                                TilepileUtils
                                    .logWarning("Other Server discovered: "
                                            + address
                                            + " My address: "
                                            + TilepileUtils
                                                .getDefaultLocalAddress());
                            ...
                            TilepileUtils.logInfo("Nonserver discovered: "
                                    + address);
